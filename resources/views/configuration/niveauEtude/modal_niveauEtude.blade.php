<div x-data="{ isOpen: false }" @open-modal-niveauetude-add.window="isOpen = true" class="fixed inset-0 z-50 flex items-center justify-center" x-show="isOpen" style="display: none;">
    ...
    <div class="relative bg-white rounded-lg shadow-2xl max-w-lg w-full mx-4 overflow-hidden" @click.stop>
        <div class="px-6 py-4 border-b border-slate-200 flex items-center justify-between">
            <h3 class="text-lg font-bold text-text-primary">Ajouter un niveau d'étude</h3>
            <button @click="isOpen = false" class="text-text-secondary hover:text-text-primary text-2xl leading-none">&times;</button>
        </div>
        <form onsubmit="return submitNiveauEtudeAdd(event)" class="p-6">
            <div class="mb-6">
                <label class="block text-sm font-medium text-text-primary mb-2">Niveau d'étude</label>
                <input type="text" name="label" required placeholder="Niveau d'étude"
...
<script>
function submitNiveauEtudeAdd(e) {
    e.preventDefault();
    fetch('{{ url('add/niveauEtude/addniveauEtude') }}', {
        method: 'POST',
