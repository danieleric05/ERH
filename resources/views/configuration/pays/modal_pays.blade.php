<div x-data="{ isOpen: false }" @open-modal-pays-add.window="isOpen = true" class="fixed inset-0 z-50 flex items-center justify-center" x-show="isOpen" style="display: none;">
    ...
    <div class="relative bg-white rounded-lg shadow-2xl max-w-lg w-full mx-4 overflow-hidden" @click.stop>
        ...
        <form onsubmit="return submitPaysAdd(event)" class="p-6">
            ...
            <div class="mb-6">
                ...
                <input type="text" name="nationalite" required placeholder="ex: IVOIRIENNE"
                       class="w-full px-4 py-2 border border-slate-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-primary-accent">
            </div>
...
<script>
function submitPaysAdd(e) {
    e.preventDefault();
    fetch('{{ url('add/pays/addpays') }}', {
        method: 'POST',
