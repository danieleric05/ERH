<div x-data="{ isOpen: false }" @open-modal-categorie-edit.window="isOpen = true" class="fixed inset-0 z-50 flex items-center justify-center" x-show="isOpen" style="display: none;">
    ...
    <div class="relative bg-white rounded-lg shadow-2xl max-w-lg w-full mx-4 overflow-hidden" @click.stop>
        <div class="px-6 py-4 border-b border-slate-200 flex items-center justify-between">
            <h3 class="text-lg font-bold text-text-primary">Modifier la catégorie</h3>
            <button @click="isOpen = false" class="text-text-secondary hover:text-text-primary text-2xl leading-none">&times;</button>
        </div>
        <form onsubmit="return submitCategorieEdit(event)" class="p-6">
            <input type="hidden" name="id" id="categorieEditId">
            <div class="mb-6">
                <label class="block text-sm font-medium text-text-primary mb-2">Catégorie</label>
                <input type="text" name="label" id="categorieEditLabel" required
...
<script>
function openCategorieEdit(id, label) {
    document.getElementById('categorieEditId').value = id;
    document.getElementById('categorieEditLabel').value = label;
    window.dispatchEvent(new CustomEvent('open-modal-categorie-edit'));
}
function submitCategorieEdit(e) {
    e.preventDefault();
    fetch('{{ url('update/categories/updatecategories') }}', {
        method: 'POST',
